...
`,type:`str
`,default:null}},raises:{},return:{name:"",type:""}};var Un={w:Rn,h:An,wh:Tn,mw:Cn,pw:Dn,color:Nn,opacity:On,tooltip:kn,to:Ln,rx:Bn,nox:In,noy:Fn,title:Hn,colormap:Pn,qcolormap:jn,save_img:$n};const qn={funcdef:"def _load_csv(url, **kwargs) -> pd.DataFrame",description:null,long_description:null,example:null,params:{},raises:{},return:{name:"",type:""}},Kn={funcdef:"def _load_django(query) -> pd.DataFrame",description:null,long_description:null,example:null,params:{},raises:{},return:{name:"",type:""}},Gn={funcdef:"def from_df(df: pd.DataFrame) -> DataSpace",description:`Intialize a DataSpace from a pandas DataFrame
...
`}},zn={funcdef:"def from_csv(url, **kwargs) -> DataSpace",description:`Loads csv data in the main dataframe
...
`,type:null,default:null}},raises:{},return:{name:null,type:`<tt class="docutils literal">DataSpace</tt>
`}},Qn={funcdef:"def from_django(query) -> DataSpace",description:`Load the main dataframe from a django orm query
...
`}};var Wn={_load_csv:qn,_load_django:Kn,from_df:Gn,from_csv:zn,from_django:Qn};const Xn="return dataspace.from_df([1])",Yn=`ds = await load_dataset("bitcoin")
ds.show()`,Vn=`ds = await load_dataset("bitcoin")
print(ds.cols_())
ds.show()`,Jn=`data = {"col1": [1, np.nan, 2, None, 3, 3]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
n = ds.count_null_("col1")
print("The column has", n, "nulls")
ds.show()`,Zn=`data = {"col1": ["foo", "", "bar", ""]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
n = ds.count_empty_("col1")
print("The column has", n, "empty values")
ds.show()`,es=`data = {"col1": [0, 1, 2, 0, 3, 3]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
n = ds.count_zero_("col1")
print("The column has", n, "zero values")
ds.show()`,ts=`data = {"col1": [1, 2, 2, 3, 3, 3]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
n = ds.count_unique_("col1")
print("The column has", n, "unique values")
ds.show()`,ns=`data = {"col1": ["one", "two", "two", "three", "three", "three"]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
df = ds.wunique_("col1")
print("Dataframe of unique values weights:")
print(df)`,ss=`df = pd.DataFrame(np.linspace(1, 100, 1000))
ds = dataspace.from_df(df)
print("Initial length:", len(ds.df.index))
ds.limit(10)
print("New length after limiting:", len(ds.df.index))
ds.show()`,as=`data = {"col1": ["A", "B", "C", "A", "B"]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
uv = ds.unique_("col1")
print("Colum unique values:", uv)
ds.show()`,is=`data = {"col1": [14, 8, np.nan], "col2": [2, np.nan, np.nan]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
print("The col1 has", ds.count_null_("col1"), "nulls")
ds.drop_nan("col1")
ds.show()`,rs=`data = {"col1": [14, 8, np.nan], "col2": [2, np.nan, np.nan]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
ds.count_null_("col1")
ds.fill_nan("val", "col1")
ds.show()`,os=`data = {"col1": np.array([1, None, ""]), "col2": np.array([None, 0, None])}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
ds.count_empty_("col1")
ds.count_null_("col1")
ds.fill_nulls()
ds.show()`,ls=`ds = await load_dataset("timeserie")
print(ds.df.head())
ds.fdate("date", precision="D")
ds.show()`,cs=`ds = await load_dataset("bitcoin")
...
print(ds.df.head())
ds.timestamps("date")
ds.show()`,us=`data = {"col1": ["5", "8", "3"], "col2": ["8", "7", "2"]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
print("Column types:", ds.cols_())
ds.to_int("col1", "col2")
print("Column types after convert:", ds.cols_())
ds.show()`,fs=`data = {"col1": ["0.25", "0.85", "0.58"]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
print("Column types:", ds.cols_())
ds.to_float("col1")
print("Column types after convert:", ds.cols_())
ds.show()`,ps=`data = {"col1": [1, 0, 0]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
print("Column types:", ds.cols_())
ds.to_type(bool, "col1")
print("Column types after convert:", ds.cols_())
ds.show()`,hs=`data = {"col1": [" one ", "two "]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
print(list(ds.df.col1))
ds.strip("col1")
print(list(ds.df.col1))
ds.show()`,gs=`data = {"col1 ": [1, 2], " col2 ": [3, 4]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
print(list(ds.df.columns.values))
ds.strip_cols()
print(list(ds.df.columns.values))
ds.show()`,_s=`data = {"col1": [1.25889, 1.25874, 1.42587]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
ds.roundvals("col1")
ds.show()`,ms=`data = {"col1": ["a", "b", "novalue"]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
ds.replace("col1", "novalue", "c")
ds.show()`,bs=`data = {"col1": ["a", "b", "c"], "col2": [12, 7, 5]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
ds.index("col1")
ds.show()`,ws=`ds = await load_dataset("timeserie")
ds.dateindex("date")
ds.show()`,ys=`ds = await load_dataset("bitcoin")
print("Unique values in the sources column:", ds.unique_("Source"))
dss = ds.split_("Source")
print("splitted DataSpace objects:", dss.keys())
dss["FTX"].show()`,Es=`data = {"col1": ["A", "B", "C", "D"]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
ds.indexcol("id")
ds.show()`,xs=`data = {"col1": [14, 8, 12], "col2": [0, 1, 0]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
ds.drop("col2")
ds.show()`,vs=`df = pd.DataFrame(np.linspace(1, 100, 5), columns=["num"])
ds = dataspace.from_df(df)
print("Adding a column with default value")
ds.add("num2", 1)
ds.show()`,Ss=`data = {"col1": [0, 1, 0]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
ds.rename("col1", "new name")
ds.show()`,Ms=`data = {"col1": [0, 1, 0], "col2": [0, 0, 1], "col3": [1, 1, 1], "col4": [0, 0, 1]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
ds.keep("col1", "col4")
ds.show()`,Rs=`data = {"col1": [14, 8, 12], "col2": [0, 1, 0]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
ds.copycol("col2", "new col name")
ds.show()`,As=`data = {"col1": [14, 25, 3, 8, 12]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
ds.sort("col1")
ds.show()`,Ts=`data = {"col1": [1, 0, 1, 4, 5, 1]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
ds.exclude("col1", 1)
ds.show()`,Cs=`data = {"col1": [1, 2, 3, 4, 5]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
ds.dropr([0, 4])
ds.show()`,Ds=`data = {"col1": [1, 2], "col2": ["foo", "bar"]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
ds.append(0, "baz")
...
ds = dataspace.from_df(df)
ds.reverse()
ds.show()`,Os=`data = {"col1": [1, 2, 3, 4, 5]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
...
ds.apply(f)
ds.show()`,ks=`ds = await load_dataset("bitcoin")
ds.keep("mktTS", "qty")
ds.dateindex("mktTS")
ds.rsum("1S", "Datapoints per second")
ds.rename("qty", "Market volume")
ds.show()`,Ls=`ds = await load_dataset("bitcoin")
ds.rmean("1S", "Datapoints per second", dateindex="mktTS")
ds.rename("px", "Mean price")
ds.show()`,Bs=`ds = await load_dataset("sp500")
ds.axis("date:T", "price:Q")
ds.line_()`,Is=`data = {"col1": ["A", "B", "C", "D", "E"], "col2": [1, 6, 2, 4, 1]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
ds.bar_("col1:N", "col2:Q")`,Fs=`ds = await load_dataset("sp500")
ds.axis("date:T", "price:Q")
ds.line_()`,Hs=`ds = await load_dataset("sp500")
ds.axis("date:T", "price:Q")
ds.point_()`,Ps=`ds = await load_dataset("sp500")
ds.axis("date:T", "price:Q")
ds.area_()`,js=`data = {"col1": ["A", "B", "C", "D", "E"], "col2": [1, 6, 2, 4, 1]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
chart = ds.bar_("col1:N", "col2:Q")
hline = ds.hline_(style={"color": "green"})
chart + hline`,$s=`data = {"col1": [1, 2, 3, 4, 4, 6, 8, 10, 12]}
df = pd.DataFrame(data)
ds = dataspace.from_df(df)
ds.diffm("col1")
ds.indexcol("id")
...
c + d`;var Us={load_dataset:Xn,show:Yn,cols_:Vn,count_null_:Jn,count_empty_:Zn,count_zero_:es,count_unique_:ts,wunique_:ns,limit:ss,unique_:as,drop_nan:is,fill_nan:rs,fill_nulls:os,fdate:ls,to_date:cs,timestamps:ds,to_int:us,to_float:fs,to_type:ps,strip:hs,strip_cols:gs,roundvals:_s,replace:ms,index:bs,dateindex:ws,split_:ys,indexcol:Es,drop:xs,add:vs,rename:Ss,keep:Ms,copycol:Rs,sort:As,exclude:Ts,dropr:Cs,append:Ds,reverse:Ns,apply:Os,rsum:ks,rmean:Ls,axis:Bs,bar_:Is,line_:Fs,point_:Hs,area_:Ps,hline_:js,diffm:$s};const qs={class:"text-xl"},Ks={class:"mt-5"},Gs={key:0},zs=M("div",{class:"p-5 pl-8 text-lg italic"},"Example",-1),Qs={key:0,class:"w-full p-3"},Vs=ke({__name:"MethodView",setup(e){const t=ct({name:"",docstring:{}}),n=K(""),d=K({}),w=K(!1),m=K(!1);function r(){var R,D,T;m.value=!1;const o=(R=xe.currentRoute.value.params)==null?void 0:R.name;t.name="",w.value=!1,d.value={};let h;if(o)h=o.toString();else return;const _=(D=xe.currentRoute.value.meta)==null?void 0:D.source;let S;_=="chart"?S=Un[h]:_=="toplevel"?S=Wn[h]:S=ot[h],t.name=h,t.docstring=S,t.docstring.example?n.value=t.docstring.example:n.value=(T=Us[h])!=null?T:"",m.value=!0}const a=Le(()=>r());return dt(()=>a()),(o,h)=>(O(),k(Be,null,[M("div",qs," Method "+ut(t.name),1),M("div",Ks,[Se(Mn,{method:t},null,8,["method"])]),n.value.length>0?(O(),k("div",Gs,[zs,m.value?(O(),k("div",Qs,[Se(ft,{id:t.name,code:n.value},null,8,["id","code"])])):G("",!0)])):G("",!0)],64))}});export{Vs as default};
